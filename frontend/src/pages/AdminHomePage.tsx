...
function AdminHomePage() {
  ...
  const onCreateSeller = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage((error as Error).message || 'Failed to create user.');
    }
  };
  ...
  return (
    <main className="mx-auto max-w-6xl">
      <section className="rounded-3xl border border-slate-200 bg-white/95 p-8 shadow-[0_16px_48px_rgba(15,23,42,0.08)]">
        ...
        <p className="mt-2 text-sm text-slate-500">All regular users are sellers with their own public pages.</p>
        ...
        <form onSubmit={onCreateSeller} className="mt-6 grid gap-4 rounded-2xl border border-slate-200 bg-slate-50/70 p-5">
          ...
          <label className="grid gap-1.5 text-sm font-medium text-slate-700">
            ...
            <input
              ...
              value={sellerForm.publicUserId}
              onChange={(event) => setSellerForm((current) => ({ ...current, publicUserId: event.target.value.toLowerCase() }))}
          ...
          {!loading ? (
            <div className="mt-4 grid gap-3">
              {users.map((item) => (
                ...
                >
                  <div>
                    <strong className="text-base font-semibold text-slate-900">@{item.publicUserId}</strong>
